...
// Navigation segments for the dashboard
const segments = [
  { id: 'portfolio', label: 'Risk Management & ROI Portfolio', icon: '📊' },
  { id: 'screener', label: 'Screener & Trade Entries', icon: '🔍' },
...
export default function AlgoTradeUI() {
      ...
      <AppBar position="static">
        <Container maxWidth="xl">
          <Toolbar disableGutters>
            ...
            {/* Segment Navigation */}
            <Box sx={{ flexGrow: 0, display: 'flex', gap: 1, mr: 2 }}>
              {segments.map((segment) => (
                <Button
                  ...
                  size="small"
                  sx={{
                    color: activeSegment === segment.id ? 'primary.main' : 'white',
                    borderColor: 'white',
                    backgroundColor: activeSegment === segment.id ? 'white' : 'transparent',
                    '&:hover': {
                      backgroundColor: activeSegment === segment.id ? 'grey.100' : 'rgba(255,255,255,0.1)',
                    },
                    fontSize: '0.8rem',
                    px: 1,
                    py: 0.5,
  ...
  const [activeSegment, setActiveSegment] = useState('portfolio');
  ...
  // --- Segment Components ---
  const RiskManagementPortfolioSegment = () => (
  ...
  const ChartsSegment = () => (
    <div style={{ marginTop: 32, display: 'flex', flexDirection: 'row', justifyContent: 'center', gap: '48px', width: '100%' }}>
  ...
  const ScreenerTradeEntriesSegment = () => (
    <>
      {/* Screener and Trade Entries Section */}
      <div className="flex flex-row gap-8 mb-8" style={{ justifyContent: 'flex-start', marginLeft: '10%', width: '100%' }}>
      ...
      <div className="w-full max-w-full mx-auto space-y-8" style={{ maxWidth: 'calc(100vw - 40px)' }}>
        <div className="bg-white rounded-lg shadow p-6" style={{ color: '#2563eb', border: '2px solid #222', borderRadius: '12px', marginLeft: '21px', maxWidth: 'calc(100vw - 60px)', overflow: 'hidden' }}>
          ...
          {/* Trade Entries Table Content */}
          <div style={{ textAlign: 'center', padding: '20px', color: '#666' }}>
            Trade entries table with all columns would go here...
            <br />
            <small>Full table implementation available in original component</small>
          </div>
        </div>
      </div>
    </>
  ...
  const SettingsSegment = () => (
    <div className="w-full max-w-4xl mx-auto">
      <div className="bg-white rounded-lg shadow p-6" style={{ color: '#2563eb', border: '2px solid #222', borderRadius: '12px', margin: '20px' }}>
        <h2 className="text-2xl font-bold mb-6" style={{ color: '#2563eb', textAlign: 'center' }}>Settings</h2>
        
        {/* User Information Section */}
        <div className="mb-8">
          <h3 className="text-lg font-semibold mb-4" style={{ color: '#2563eb' }}>User Information</h3>
        ...
        <div className="mb-8">
          <h3 className="text-lg font-semibold mb-4" style={{ color: '#2563eb' }}>Risk Management Settings</h3>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Diversification:</label>
              <input 
                type="number" 
  ...
  // Function to render active segment
  const renderActiveSegment = () => {
    switch(activeSegment) {
      case 'portfolio':
        return <RiskManagementPortfolioSegment />;
      case 'screener':
        return <ScreenerTradeEntriesSegment />;
      case 'charts':
        return <ChartsSegment />;
      case 'settings':
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center" style={{ background: 'linear-gradient(135deg, #e0e7ff 0%, #f8fafc 100%)', minHeight: '100vh', width: '100vw', padding: '32px 0' }}>
        ...
        {/* Segment Navigation Pills - Mobile Friendly */}
        <div className="w-full mb-8" style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '12px', paddingTop: '20px' }}>
          {segments.map((segment) => (
            <button
              key={segment.id}
              className={`segment-btn ${activeSegment === segment.id ? 'active' : ''}`}
              style={{
                background: activeSegment === segment.id ? '#2563eb' : 'white',
                color: activeSegment === segment.id ? 'white' : '#2563eb',
                ...
                boxShadow: activeSegment === segment.id ? '0 4px 12px rgba(37,99,235,0.3)' : '0 2px 8px rgba(0,0,0,0.1)',
                transform: activeSegment === segment.id ? 'translateY(-2px)' : 'none',
                minWidth: '200px',
                display: 'flex',
                alignItems: 'center',
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>{segment.icon}</span>
